...
class TenantUser(models.Model):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="tenantUsers"
    )
    ...
    def get_owner(self):
        return self.tenantUsers.filter(role=TenantUser.OWNER).first()
    ...
    def save(self, *args, **kwargs):
        if self.pk:
            existing_default = TenantUser.objects.filter(
        ...
        if existing_default.exists() and self.is_default:
    ...
    Tenant.add_to_class("owner", get_owner)
